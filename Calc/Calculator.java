...
public void compute() {
    if (this.currentOperand.isEmpty() || this.previousOperand.isEmpty()) {
        return;
    }

    double curr = Double.parseDouble(this.currentOperand);
    double prev = Double.parseDouble(this.previousOperand);
    
    try {
        Operation operation = OperationFactory.createOperation(this.operation);
        double result = operation.calculate(prev, curr);
        
        // ✅ حفظ العملية في التاريخ - Singleton في العمل!
        String expression = String.format("%s %s %s", prev, this.operation, curr);
        CalculationHistory.getInstance().addCalculation(expression, result);
...
// دالة لعرض آخر عملية
public void showLastCalculation() {
    CalculationHistory.CalculationRecord last = 
        CalculationHistory.getInstance().getLastCalculation();
    
    if (last != null) {
        this.currentOperand = last.toShortString();
    } else {
        this.currentOperand = "No history";
    }
    this.updateDisplay();
}

// دالة لعرض عدد العمليات المحفوظة
public void showHistoryCount() {
    int count = CalculationHistory.getInstance().getHistorySize();
    this.currentOperand = "History: " + count + " operations";
    this.updateDisplay();
}

// دالة لمسح التاريخ
public void clearHistory() {
    CalculationHistory.getInstance().clearHistory();
    this.currentOperand = "History cleared";
    this.updateDisplay();
}

// دالة لعرض كل التاريخ (مختصر)
public void showFullHistory() {
    List<CalculationHistory.CalculationRecord> history = 
        CalculationHistory.getInstance().getHistory();
    
    if (history.isEmpty()) {
        this.currentOperand = "No history available";
    } else {
        // عرض آخر 3 عمليات كمثال
        StringBuilder sb = new StringBuilder();
        int start = Math.max(0, history.size() - 3);
        for (int i = start; i < history.size(); i++) {
            sb.append(history.get(i).toShortString());
            if (i < history.size() - 1) sb.append("; ");
        }
        this.currentOperand = sb.toString();
    }
    this.updateDisplay();
}
